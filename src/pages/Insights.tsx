  ...
  return (
    <>
      <header className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <span className="text-primary font-headline font-bold tracking-[0.2em] text-xs uppercase mb-2 block">Performance Core</span>
          <h1 className="text-5xl md:text-6xl font-headline font-extrabold tracking-tighter leading-[0.9] text-white">Refined Insights <br/>& Analytics.</h1>
        </div>
        <div className="flex bg-surface-container p-1 rounded-full border border-white/5">
          <button className="px-6 py-2 rounded-full bg-surface-container-high text-primary font-headline font-bold text-sm shadow-sm transition-all cursor-pointer">Week</button>
          <button className="px-6 py-2 rounded-full text-on-surface-variant font-headline font-bold text-sm hover:text-white transition-all cursor-pointer">Month</button>
        </div>
      </header>
      <div className="grid grid-cols-12 gap-8 items-start">
        <div className="col-span-12 xl:col-span-8 bg-surface-container-low rounded-[2rem] p-8 md:p-12 relative overflow-hidden group">
          <div className="flex items-center justify-between mb-12 relative z-10">
            <div>
              <h2 className="font-headline font-bold text-2xl text-white">Focus Hours</h2>
              <p className="text-on-surface-variant font-body text-sm mt-1">Deep work distribution across the cycle.</p>
            </div>
            <div className="text-right">
              <span className="text-3xl font-headline font-extrabold text-primary block">142.8h</span>
              <span className="text-xs text-primary-container">+12% vs last period</span>
            </div>
          </div>
        ...
        <div className="col-span-12 xl:col-span-4 flex flex-col gap-8">
          <div className="bg-primary-container/10 border border-primary/10 rounded-[2rem] p-8 transition-transform hover:scale-[1.02] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] cursor-pointer">
            <span className="material-symbols-outlined text-primary mb-6" style={{ fontSize: '40px' }}>bolt</span>
            <h3 className="font-headline font-bold text-sm text-primary uppercase tracking-[0.2em] mb-2">Peak Velocity</h3>
            <h2 className="text-4xl font-headline font-extrabold text-white leading-tight mb-8">Tuesday is your most productive day.</h2>
            <p className="text-on-surface-variant mb-10 text-lg leading-relaxed">Your output spikes between 09:00 and 11:30. Block this time to maximize focus.</p>
            <button className="flex items-center gap-2 text-primary font-headline font-bold text-sm group cursor-pointer">
              Schedule Focus Block
              <span className="material-symbols-outlined text-sm group-hover:translate-x-1 transition-transform">arrow_forward</span>
            </button>
          </div>
          <div className="bg-surface-container-high rounded-[2rem] p-8 flex items-center gap-6">
            <div className="w-16 h-16 rounded-2xl bg-surface-container flex items-center justify-center text-primary">
              <span className="material-symbols-outlined" style={{ fontSize: '32px' }}>timer</span>
            </div>
            <div>
              <p className="text-on-surface-variant text-sm font-medium">Session Avg</p>
              <h4 className="text-3xl font-headline font-bold text-white leading-none mt-1">4h 12m</h4>
            </div>
          </div>
          <div className="bg-surface-container-high rounded-[2rem] p-8 flex items-center gap-6">
            <div className="w-16 h-16 rounded-2xl bg-surface-container flex items-center justify-center text-tertiary">
            ...
            <div>
              ...
              <h4 className="text-3xl font-headline font-bold text-white leading-none mt-1">94.2%</h4>
            </div>
          </div>
        </div>
